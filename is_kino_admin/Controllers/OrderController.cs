...
using is_kino_admin.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json;
using System.Text;

namespace is_kino_admin.Controllers
{
    public class OrderController : Controller
    {
        ...
        public IActionResult CreateInvoice(Guid id)
        {
            ...
            foreach (var item in result.TicketInOrders)
            {
                ...
                sb.AppendLine(item.OrderedTicket.Id + " with quantity of: " + item.Quantity + " and price of: $" + item.OrderedTicket.Price);
            }
            ...
            using (var workBook = new XLWorkbook())
            {
                ...
                for (int i = 1; i <= result.Count(); i++)
                {
                    ...
                    for (int p = 1; p <= item.TicketInOrders.Count(); p++)
                    {
                        ...
                        worksheet.Cell(i + 1, p + 4).Value = item.TicketInOrders.ElementAt(p - 1).OrderedTicket.Id.ToString();
                    }
